...
class GetGalleryPhotosHandlerTest : AbstractHandlerTest() {
			...
			"/v1".nest {
				"/api".nest {
					accept(MediaType.APPLICATION_JSON).nest {
						GET("/get_gallery_photos/{photo_ids}", handler::handle)
					}
				}
			}
		})
	...
	fun `photos should be sorted by id in descending order`() {
		...
		runBlocking {
			...
			photoInfoDao.save(PhotoInfo(2, 2, 4L, "111", "222", "photo2", true, 11.1, 11.1, 6L)).awaitFirst()
			photoInfoDao.save(PhotoInfo(3, 3, 2L, "111", "222", "photo3", true, 11.1, 11.1, 5L)).awaitFirst()
		...
		kotlin.run {
			...
			assertEquals(5, response.galleryPhoto.size)

			assertEquals(5, response.galleryPhoto[0].id)
			assertEquals(4, response.galleryPhoto[1].id)
			assertEquals(3, response.galleryPhoto[2].id)
			assertEquals(2, response.galleryPhoto[3].id)
			assertEquals(1, response.galleryPhoto[4].id)
		}
	}
}
